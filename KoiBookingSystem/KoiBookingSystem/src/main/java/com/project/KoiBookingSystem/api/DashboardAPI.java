...
public class DashboardAPI {
    ...
    public ResponseEntity<Long> getOrdersByMonth(@RequestParam int month, @RequestParam int year) {
        long orderCount = dashboardService.countOrdersByMonth(month, year);
        return ResponseEntity.ok(orderCount);
    }

    // Số lượng đơn hàng trong năm
    @GetMapping("/countOrders/year")
    public ResponseEntity<Long> getOrdersThisYearCount(@RequestParam int year) {
        return ResponseEntity.ok(dashboardService.  countOrdersThisYear(year));
    }
    ...
    public ResponseEntity<Long> countCheckedBookings() {
        long count = dashboardService.countCheckedBookings();
        return ResponseEntity.ok(count);
    }
    // Số lượng vé ma khách hàng booked ở tour đó ( tour có sẵn: 10 vé)
    @GetMapping("/checked-bookings/tour-available/{tourId}")
    public ResponseEntity<Long> countCheckedBookingsForAvailableTour(@PathVariable String tourId) {
        long checkedBookingsCount = dashboardService.countCheckedBookingsForAvailableTour(tourId);
        if (checkedBookingsCount < 0) {
            return ResponseEntity.notFound().build(); // Nếu không tìm thấy
        }
        return ResponseEntity.ok(checkedBookingsCount);
    }
    // Số lượng vé ma khách hàng booked bởi tour có yêu cầu ( tour yêu cầu: 1 vé)
    @GetMapping("/checked-bookings/tour-request/{tourId}")
    public ResponseEntity<Long> countCheckedBookingsForRequestTour(@PathVariable String tourId) {
        long checkedBookingsCount = dashboardService.countCheckedBookingsForRequestTour(tourId);
        if (checkedBookingsCount < 0) {
            return ResponseEntity.notFound().build(); // Nếu không tìm thấy
        }
        return ResponseEntity.ok(checkedBookingsCount);
