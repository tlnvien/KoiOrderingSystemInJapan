...
public class OrderService {
    ...
    public OrderResponse createNewOrder(OrderRequest orderRequest, String tourId, String customerId) {
        ...
        List<OrderDetail> orderDetails = createOrderDetails(orderRequest, newOrder);
    ...
    public List<OrderResponse> getAllOrdersByFarmHost() {
        ...
        Farm farm = farmRepository.findFarmByFarmIdAndStatusTrue(farmHost.getFarm().getFarmId());
    ...
    private Koi findKoi(String species) {
        Koi koi = koiRepository.findBySpeciesAndStatusTrue(species);
        if (koi == null) {
    ...
    private List<OrderDetail> createOrderDetails(OrderRequest orderRequest, Orders order) {
        Set<String> uniqueDetails = new HashSet<>();

        return orderRequest.getOrderDetails().stream().map(orderDetailRequest -> {

            String uniqueKey = orderDetailRequest.getSpecies() + " | "
                    + orderDetailRequest.getDescription() + " | "
                    + orderDetailRequest.getQuantity() + " | "
                    + orderDetailRequest.getPrice();

            if (!uniqueDetails.add(uniqueKey)) {
                throw new InvalidRequestException("Thông tin đơn hàng mà bạn nhập có sự trùng lặp!");
            }
    ...
    public List<OrderDetailResponse> getAllOrderDetailByOrder(String orderId) {
        List<OrderDetail> orderDetails = orderDetailRepository.findAllByOrderId(orderId);
        if (orderDetails.isEmpty()) {
            throw new EmptyListException("Chi tiết của đơn hàng này không tồn tại!");
        }
        return orderDetails.stream().map(this::convertToOrdersResponse).collect(Collectors.toList());
    }

    public OrderDetailResponse convertToOrdersResponse(OrderDetail orderDetail) {
        OrderDetailResponse orderDetailResponse = new OrderDetailResponse();
        orderDetailResponse.setSpecies(orderDetail.getKoi().getSpecies());
        orderDetailResponse.setDescription(orderDetail.getDescription());
        orderDetailResponse.setQuantity(orderDetail.getQuantity());
        String formattedTotalPrice = NumberFormat.getCurrencyInstance(new Locale("vi", "VN")).format(orderDetail.getPrice());
        orderDetailResponse.setPrice(formattedTotalPrice);
        return orderDetailResponse;
    }
