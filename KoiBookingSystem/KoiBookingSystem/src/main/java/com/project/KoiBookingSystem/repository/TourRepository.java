...
public interface TourRepository extends JpaRepository<Tour, Long> {
    ...
    @Query("SELECT t FROM Tour t WHERE t.status = com.project.KoiBookingSystem.enums.TourStatus.NOT_YET")
    List<Tour> findTourForBooking();
    ...
    "LEFT JOIN kf.koi k " +
    "WHERE t.status <> com.project.KoiBookingSystem.enums.TourStatus.CANCELLED " +
    "AND (:tourName IS NULL OR t.tourName LIKE CONCAT ('%', :tourName, '%')) " +
    "AND (:farmName IS NULL OR f.farmName LIKE CONCAT ('%', :farmName, '%')) " +
    "AND (:koiSpecies IS NULL OR k.species LIKE CONCAT ('%', :koiSpecies, '%')) " +
    "AND (:departureMonth IS NULL OR FUNCTION('MONTH', t.departureDate) = :departureMonth) " +
    "AND (:departureYear IS NULL OR FUNCTION('YEAR', t.departureDate) = :departureYear) " +
    "AND (:minPrice IS NULL OR t.price >= :minPrice) " +
    "AND (:maxPrice IS NULL OR t.price <= :maxPrice)" +
    "ORDER BY DAY(t.departureDate), MONTH(t.departureDate), YEAR(t.departureDate)")
    List<Tour> searchTours(@Param("tourName") String tourName,
                           @Param("farmName") String farmName,
                           @Param("koiSpecies") String koiSpecies,
                           @Param("departureMonth")Integer departureMonth,
                           @Param("departureYear") Integer departureYear,
    ...
    @Query("SELECT t FROM Tour t WHERE t.status = com.project.KoiBookingSystem.enums.TourStatus.COMPLETED")
    List<Tour> findByTourCompleted();

    @Query("SELECT t FROM Tour t WHERE t.status = com.project.KoiBookingSystem.enums.TourStatus.IN_PROGRESS")
    List<Tour> findByTourInProgress();

    @Query("SELECT t FROM Tour t JOIN t.bookings b WHERE b.customer.userId = :customerId")
    List<Tour> findTourByCustomerId(@Param("customerId") String customerId);
