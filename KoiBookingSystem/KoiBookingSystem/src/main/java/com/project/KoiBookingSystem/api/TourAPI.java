...
public class TourAPI {
    ...
    public ResponseEntity getTourHistory() {
        List<TourResponse> tourResponses = tourService.getTourHistory();
        return ResponseEntity.ok(tourResponses);
    }
    ...
    @PreAuthorize("hasAnyAuthority('CONSULTING', 'MANAGER', 'SALES', 'CUSTOMER')")
    public ResponseEntity getTourByCustomer(@PathVariable String customerId) {
        List<TourResponse> tourResponses = tourService.getTourByCustomer(customerId);
        return ResponseEntity.ok(tourResponses);
    }
}
