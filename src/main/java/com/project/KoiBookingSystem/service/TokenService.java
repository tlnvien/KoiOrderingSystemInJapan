...
public class TokenService {
    ...
    public String generateToken(Account account) {
        String token = Jwts.builder()
                .subject(account.getUsername() + "") // không trùng và không thể thay đổi
                .issuedAt(new Date(System.currentTimeMillis())) // lấy thời gian tạo ra token
                .expiration(new Date(System.currentTimeMillis() + 1000 * 60 * 60 * 24)) // set thời gian hết hạn token
                .signWith(getSignInKey()) // gắn vào jwt
                .compact();
        return token;
    }
