      ...
      <div className="order-list" style={{ marginTop: 16 }}>
        {orders.length > 0 ? (
          orders.map((order) => (
            ...
            >
              <Row gutter={16}>
                <Col span={6}><strong>Mã Đơn Hàng:</strong> {order.orderId}</Col>
                <Col span={6}><strong>Mã Khách Hàng:</strong> {order.customerId}</Col>
                <Col span={6}><strong>Mã Tour:</strong> {order.tourId}</Col>
                <Col span={6}><strong>Tổng Giá:</strong> {order.totalPrice.toLocaleString()} VND</Col>
              </Row>
              <Divider />
              <Row gutter={16}>
                <Col span={6}><strong>Ngày Đặt:</strong> {order.orderDate}</Col>
                <Col span={6}><strong>Trạng Thái:</strong> {order.status}</Col>
                <Col span={12}><strong>Ghi Chú:</strong> {order.note}</Col>
              </Row>
              <h4 style={{ marginTop: 16 }}>Chi Tiết Đơn Hàng:</h4>
              {order.orderDetails.map((detail) => (
                <div key={detail.koiId} className="order-detail">
                  <p><strong>Mã Koi:</strong> {detail.koiId}</p>
                  <p><strong>Mô Tả:</strong> {detail.description}</p>
                  <p><strong>Số Lượng:</strong> {detail.quantity}</p>
                  <p><strong>Giá:</strong> {detail.price.toLocaleString()} VND</p>
                  <Divider />
                </div>
              ))}
